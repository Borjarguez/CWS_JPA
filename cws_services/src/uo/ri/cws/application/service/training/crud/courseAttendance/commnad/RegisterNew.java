...
//		TODO revisar esto por si acaso, en JDBC buscaba por courseID y mechanicID
//		BusinessCheck.isNull(repo.findById(dto.id), "Attendance already exists");
		BusinessCheck.isTrue(m.isPresent(), "Mechanic does not exist");
		BusinessCheck.isNull(c.isPresent(), "Course does not exist");
		BusinessCheck.isTrue(dto.attendance < 85 && dto.passed, "Attendance to pass must be over 85%");
		BusinessCheck.isTrue(dto.attendance < 0, "Attendance must be over 0");
		BusinessCheck.isTrue(dto.attendance > 100, "Attendance must be under 100");
		
		Enrollment e = new Enrollment(m.get(), c.get(), dto.attendance, dto.passed);
		assert(e!=null);
		repo.add(e);
		return DtoAssembler.toDto(e);
	}
